$(document).ready(function(){
  var input = document.getElementById('input');// reference to our  input box
 ...
 var makeTaskHtml = function(str, onCheck){ // function who whill create the list elements that we gonna inject inside the lists, the first parameter is the string thats gonna be added as tasks and the second is the callback function onCheck we can see the use of it below
    ...
    // label.textContent.appendChild(new Date());
  ...
  var addTask = function(task){// to function that will add the task to the  list todo element above
  	  lists.todo.appendChild(task); //take and element and append it to the subtree of the element(list  todo in our case) ==> it will createa an "il" element in the "ul" todo
  };
  ...
  var onCheck = function(event){
  	...
  	lists[list === 'done' ? 'todo' : 'done'].appendChild(task);//it will remove this elemen(task) from where it was an appended in our element because it cant be in two places in the same time in the DOM, if the value of the id is 'done' then append it to the ul todo or incompleted list else append it to our done list of completed task, this is condition inside a key (really amasing) we must use the square bracket cause its a variable
...
var onInput = function(){
	var str = input.value.trim();// this built in function trim() remove the white space from the input if for example the words are preceded by white space
	
	if(str.length > 0){// if the input is letter at least not empty 
		addTask(makeTaskHtml(str, onCheck));// the first parameter of our function add task is the list created by our function makeTaskHtml( our function makeTaskHtml will take as parameter the str entered as input and the call back function an return the list to do ) 
...
input.addEventListener('keyup', function(event){// this function will improve the way of entering the input so we don't have each to click add we can type "enter" too
